...
def generate_seed_fullscan(viewangle=360, n_views=10, upper_angle=30, lower_angle=-30):
    """
    Generate full space scanning poses.
    
    Parameters:
        viewangle (int): Total angle to cover in the horizontal plane.
        n_views (int): Number of views in each horizontal circle.
        upper_angle (int): The angle above the horizontal plane to scan.
        lower_angle (int): The angle below the horizontal plane to scan.
    
    Returns:
        render_poses (np.ndarray): Generated poses.
    """
    N = n_views
    render_poses = []
    ...
    for i in range(N):
        th = (viewangle/N)*i/180*np.pi
        pose = np.zeros((3, 4))
        pose[:3, :3] = np.array([[np.cos(th), 0, np.sin(th)], 
                                 [0, 1, 0], 
                                 [-np.sin(th), 0, np.cos(th)]])
        pose[:3, 3:4] = np.random.randn(3,1)*0.0  # Transition vector
        render_poses.append(pose)

    # Upper circle
    for i in range(N):
        th = (viewangle/N)*i/180*np.pi
        phi = upper_angle/180*np.pi
        pose = np.zeros((3, 4))
        pose[:3, :3] = np.matmul(np.array([[np.cos(th), 0, -np.sin(th)], 
                                           [0, 1, 0], 
                                           [np.sin(th), 0, np.cos(th)]]), 
                                 np.array([[1, 0, 0], 
                                           [0, np.cos(phi), -np.sin(phi)], 
                                           [0, np.sin(phi), np.cos(phi)]]))
        pose[:3, 3:4] = np.zeros((3, 1))
        render_poses.append(pose)

    # Lower circle
    for i in range(N):
        th = (viewangle/N)*i/180*np.pi
        phi = lower_angle/180*np.pi
        pose = np.zeros((3, 4))
        pose[:3, :3] = np.matmul(np.array([[np.cos(th), 0, -np.sin(th)], 
                                           [0, 1, 0], 
                                           [np.sin(th), 0, np.cos(th)]]), 
...
def generate_seed_layerscan(viewangle=360, n_views=10, upper_angle=30, lower_angle=-30):
    N = n_views
    render_poses = []
    ...
    for i in range(N):
        ...
        pose[:3, 3:4] = np.zeros((3, 1))  # Transition vector
        render_poses.append(pose)
    ...
    # 垂直向上的扫描姿态矩阵
    pose_up = np.zeros((3, 4))
    pose_up[:3, :3] = np.array([
        [1, 0, 0], 
        [0, 0, 1], 
        [0, -1, 0]
    ])
    pose_up[:3, 3:4] = np.zeros((3, 1))  # 平移向量
    render_poses.append(pose_up)

    return np.array(render_poses)

def generate_seed_lowerscan(viewangle=360, n_views=10, lower_angle=30):
    N = n_views
    render_poses = []
...
def get_pcdGenPoses(pcdgenpath, argdict={}):
    ...
    elif pcdgenpath == 'fullscan':
        render_poses = generate_seed_fullscan()
    elif pcdgenpath == 'layeredscan':
        render_poses = generate_seed_layerscan()
    elif pcdgenpath == 'lowerscan':
        render_poses = generate_seed_lowerscan()
    else:
        raise("Invalid pcdgenpath")
    return render_poses

def generate_seed_rotate360(nviews=360, radius=1.0):
    """
    Generate a sequence of camera poses for a 360-degree rotation around the origin.
    
    Parameters:
    nviews (int): Number of views to generate. Default is 360.
    radius (float): Radius of the circular path. Default is 1.0.
    
    Returns:
    np.ndarray: Array of shape (nviews, 3, 4) containing the camera poses.
    """
    ...
    for i, angle in enumerate(angles):
        ...
        # Translation vector
        render_poses[i, :3, 3] = np.array([radius * cos_angle, 0, radius * sin_angle])
...
def get_camerapaths():
    preset_json = {}
    for cam_path in ["back_and_forth", "llff", "headbanging","rotate360","360","1440"]:
        if cam_path == 'back_and_forth':
            ...
            render_poses = generate_seed_rotate360(nviews=360, radius=1.0)
        elif cam_path == '360':
